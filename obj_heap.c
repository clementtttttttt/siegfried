...
void* liballoc_alloc(int in){
	
	return page_find_and_alloc(in);
}

int liballoc_free(void* in, int pz){
	page_free_found((unsigned long)in, pz);
	return 0;
}
...
static unsigned int l_pageSize  = 2097152;			//< Individual page size
...
void k_obj_free(void *ptr)
{
	...
	liballoc_lock();
		...
		{
			draw_string("FREE ERROR: INVALID MAGIK\n");
...
void* k_obj_calloc(size_t nobj, size_t size)
{
       int real_size;
       void *p;
...
void*   k_obj_realloc(void *p, size_t size)
{
	void *ptr;
	struct boundary_tag *tag;
	size_t real_size;
	...
	if ( p == NULL ) return k_obj_alloc( size );
	...
	liballoc_lock();
		...
		real_size = tag->size;
		if(tag->magic != LIBALLOC_MAGIC){
				draw_string("REALLOC ERROR: INVALID MAGIK");
				while(1){}
		}
	liballoc_unlock();
