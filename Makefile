...
test: sf.iso
	qemu-system-x86_64 -S -s -cdrom sf.iso -machine q35  -no-reboot -d guest_errors,int -drive file=test.img,if=none,id=nvm -device nvme,serial=deadbeef,drive=nvm -m 128 -bios /usr/share/edk2-ovmf/OVMF_CODE.fd  -smp 3  -cpu Skylake-Client -monitor stdio
